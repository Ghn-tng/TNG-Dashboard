...
ltc_rows = ''
for i,x in enumerate(sorted(data['ltc_am'], key=lambda a: safe_num(a.get('total_ltc',0)))):
...
ltc_tts_rows = ''
for i,x in enumerate(sorted(data.get('ltc_tts',[]), key=lambda a: safe_num(a.get('total_ltc',0)))):
    cls = status_class(x.get('total_ltc',0))
...
kd_total_row = f'<tr style="font-weight:700;background:var(--card2)"><td class="text-left">Tổng cộng</td>{tl_day_cells}<td class="{tl_n1_cls}"><b>{money(tl_n1)}</b></td><td><b>{money(total_cungky)}</b></td><td><b>{money(total_rev_lay)}</b></td><td class="{tl_so_cls}"><b>{money(tl_so)}</b></td></tr>'
